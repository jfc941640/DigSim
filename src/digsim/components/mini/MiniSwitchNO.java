...
public class MiniSwitchNO extends ElectronicComponent
{

    public MiniSwitchNO(Pin apin[][], int i, int j)
    {
        super(i, j, 10, 5, 2, 1, 2, 2, 0, 0);
        MiniSwitchNOClosed = true;
        ALevel = -1;
        QLevel = -1;
        ComponentName = "Inter NO (normalement ouvert - contact T)";
        ClassName = "MiniSwitchNO";
        Can_Rotate = true;
        APos = new Point(1, 2);
        QPos = new Point(5, 2);
    ...
    public MiniSwitchNO(ElectronicComponent electroniccomponent, int i, int j)
    {
    ...
    {
        MiniSwitchNO miniswitchno = new MiniSwitchNO(this, i, j);
        miniswitchno.NbRotation = NbRotation;
        miniswitchno.updatePinsPosition();
        return miniswitchno;
    }

    public boolean SimMouseDown()
    {
        MiniSwitchNOClosed = !MiniSwitchNOClosed;
        return true;
    }
    ...
    public void draw(GraphicsContext g, int i, int j, int k)
    {
        ...
        if(NbRotation == 0)
        {
            ...
            if(MiniSwitchNOClosed)
            {
                DrawWithOffset.strokeLine(g,(l + 2) * k, (i1 + 3) * k - 1, (l + 4) * k, (i1 + 2) * k - 1);
                DrawWithOffset.strokeLine(g,(l + 2) * k, (i1 + 3) * k, (l + 4) * k, (i1 + 2) * k);
            } else
            {
                DrawWithOffset.strokeLine(g,(l + 2) * k, (i1 + 2) * k - 1, (l + 4) * k, (i1 + 2) * k - 1);
                DrawWithOffset.strokeLine(g,(l + 2) * k, (i1 + 2) * k, (l + 4) * k, (i1 + 2) * k);
            }
        } else
        if(NbRotation == 1)
        {
            ...
            if(MiniSwitchNOClosed)
            {
                DrawWithOffset.strokeLine(g,(l + 2) * k - 1, (i1 + 1) * k, (l + 3) * k - 1, (i1 + 3) * k);
                DrawWithOffset.strokeLine(g,(l + 2) * k, (i1 + 1) * k, (l + 3) * k, (i1 + 3) * k);
            } else
            {
                DrawWithOffset.strokeLine(g,(l + 3) * k - 1, (i1 + 1) * k, (l + 3) * k - 1, (i1 + 3) * k);
