...
public class CtrBcdBinRazASync extends IntegratedCircuit
{

    public CtrBcdBinRazASync(Pin apin[][], int i, int j)
    {
        super(i, j, 12, 14, 3, 1, 5, 11, 11, 5);
        Aff = 0;
        ...
        IPin[7] = new InputPin("U/D", 1, 11, 2, 0, 0, 0, 8);
        IPin[8] = new InputPin("ENT", 1, 9, 2, 0, 0, 0, 8);
        IPin[9] = new InputPin("Bin/Dec", 1, 10, 2, 0, 0, 0, 8);
        ...
        OPin[4] = new OutputPin("TC", 10, 9, -2, 0, 0, 0, 0);
        Count = Init;
        for(int k = 0; k < 11; k++)
            IPin[k].setLevel(-1);

        ComponentName = "CtrDctr BIN-DCB 4bits prog -CTR-PIPO- RAZ async";
        ClassName = "CtrBcdBinRazASync";
        Can_Rotate = false;
        RegisterPins(apin, i, j);
    }

    public CtrBcdBinRazASync(ElectronicComponent electroniccomponent, int i, int j)
    {
    ...
    {
        CtrBcdBinRazASync ctrbcdbinrazasync = new CtrBcdBinRazASync(this, i, j);
        return ctrbcdbinrazasync;
    }

    public void SimulateLogic()
    {
        Aff = 1;
        if(IPin[8].getLevel() == 5 && IPin[7].getLevel() == 5 && (IPin[9].getLevel() == 5 && Count == 15 || IPin[9].getLevel() == 0 && Count == 9) || IPin[8].getLevel() == 5 && IPin[7].getLevel() == 0 && Count == 0)
            OPin[4].setLevel(5);
        else
        if(IPin[8].getLevel() == 0 || IPin[7].getLevel() == 0 || (IPin[9].getLevel() == 0 || Count != 15 && IPin[9].getLevel() == 5 || Count != 0) && IPin[8].getLevel() == 0 || IPin[7].getLevel() == 5 || Count != 0)
            OPin[4].setLevel(0);
        ...
        if(IPin[4].getOldLevel() == 0 && IPin[4].getLevel() == 5)
            ...
            {
                Count++;
                if(IPin[9].getLevel() == 5 && BinDec == 5 && (Count < 0 || Count > 15))
                    Count = 0;
                else
                if(IPin[9].getLevel() == 0 && BinDec == 0 && (Count < 0 || Count > 9))
                    Count = 0;
            } else
