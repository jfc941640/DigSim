...
public class RailCourbe90d8x8 extends ElectronicComponent
{

    public RailCourbe90d8x8(Pin apin[][], int i, int j)
    {
        super(i, j, 12, 12, 1, 1, 7, 7, 4, 2);
        Count = 0;
        Train = false;
        Bout = false;
        COLLISION = false;
        ...
        IPin[2] = new InputPin("", 2, -1, 0, 2, 0, 0, 0);
        IPin[3] = new InputPin("", 1, 0, 0, 1, 0, 0, 0);
        OPin[0] = new OutputPin("", 8, 9, 0, -1, 0, 0, 0);
        OPin[1] = new OutputPin("", 0, 2, 1, 0, 0, 0, 0);
        ComponentName = "Rail courbe 90\260";
        ClassName = "RailCourbe90d8x8";
        Can_Rotate = true;
        RegisterPins(apin, i, j);
    }

    public RailCourbe90d8x8(ElectronicComponent electroniccomponent, int i, int j)
    {
    ...
    {
        RailCourbe90d8x8 railcourbe90d8x8 = new RailCourbe90d8x8(this, i, j);
        return railcourbe90d8x8;
    }
    ...
    public void draw(GraphicsContext g, int i, int j, int k)
    {
        ...
        if(IPin[0].PinDim.height > 0)
        {
            ...
            DrawWithOffset.strokeLine(g,(l + 8) * k, (i1 + 1) * k, (l + 7) * k, (i1 + 1) * k);
        ...
        if(IPin[0].PinDim.width < 0)
        {
            ...
            DrawWithOffset.strokeLine(g,(l + 8) * k, (i1 + 8) * k, (l + 8) * k, (i1 + 7) * k);
            DrawWithOffset.strokeLine(g,(l + 7) * k - 1, (i1 + 8) * k, (l + 7) * k - 1, (i1 + 10) * k);
            if(COLLISION)
            {
                g.setStroke(MyColor.red);
        ...
        if(IPin[0].PinDim.height < 0)
        {
            ...
            DrawWithOffset.strokeLine(g,(l + 1) * k, (i1 + 8) * k, (l + 2) * k, (i1 + 8) * k);
            DrawWithOffset.strokeLine(g,(l - 1) * k, (i1 + 7) * k - 1, (l + 1) * k, (i1 + 7) * k - 1);
            if(COLLISION)
            {
                g.setStroke(MyColor.red);
                DrawWithOffset.fillOval(g,(l + 3) * k - 4, (i1 + 4) * k, 1 * k + 1, 1 * k + 1);
