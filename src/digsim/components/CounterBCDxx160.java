...
public class CounterBCDxx160 extends IntegratedCircuit
{

    public CounterBCDxx160(Pin apin[][], int i, int j)
    {
        ...
        OPin[4] = new OutputPin("T9", 10, 9, -2, 0, 0, 0, 0);
        Count = Init;
        for(int k = 0; k < 9; k++)
            IPin[k].setLevel(-1);

        ComponentName = "Ctr DCB 4bits prog -CTR-PIPO- RAZ async (74HC160)";
        ClassName = "CounterBCDxx160";
        Can_Rotate = false;
        RegisterPins(apin, i, j);
    }

    public CounterBCDxx160(ElectronicComponent electroniccomponent, int i, int j)
    {
    ...
    {
        CounterBCDxx160 counterbcdxx160 = new CounterBCDxx160(this, i, j);
        return counterbcdxx160;
    }

    public void SimulateLogic()
    {
        Aff = 1;
        if(Count == 9 && IPin[8].getLevel() == 5)
            OPin[4].setLevel(5);
        else
        if(Count != 9 || IPin[8].getLevel() == 0)
            OPin[4].setLevel(0);
